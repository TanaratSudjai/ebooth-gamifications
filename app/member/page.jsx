    ...
    const sendCheckIn = async () => {
      if (form.activity_id && form.sub_activity_id && form.member_id) {
        try {
          ...
          // Find the activity that was scanned
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 text-gray-800 p-4 sm:p-6">
      <div className="container mx-auto max-w-4xl space-y-8">
        ...
        <header className="bg-white shadow-md rounded-lg p-6 border-l-4 border-blue-500">
          <h1 className="text-3xl font-bold text-blue-800">QR Check-in System</h1>
          <p className="text-gray-600 mt-2">Scan a QR code to check in to an activity</p>
        </header>
        ...
        <section className="bg-white shadow-lg rounded-lg p-6 relative overflow-hidden">
          ...
          {scanSuccess && (
            <div className="mb-4 bg-green-100 border-l-4 border-green-500 p-4 rounded animate-pulse">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  {selectedActivity && (
                    <p className="text-green-600 text-sm">Checked in to: {selectedActivity.sub_activity_name}</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div id="qr-reader" className="mt-4 mx-auto max-w-md rounded-lg overflow-hidden shadow-inner" />
        ...
        {session?.user && (
          <section className="bg-white shadow-lg rounded-lg p-6 relative overflow-hidden">
            ...
            <div className="flex items-center mb-4">
              ...
              <h2 className="text-2xl font-semibold text-gray-800">Member Info</h2>
            </div>
            
            <div className="bg-gradient-to-r from-green-50 to-teal-50 p-4 rounded-lg border border-green-100 mb-4">
              <div className="grid sm:grid-cols-2 gap-x-6 gap-y-3">
                <div className="flex items-center">
                  <span className="text-green-700 font-medium w-24">Username:</span>
                  <span className="text-gray-800">{session.user.username}</span>
                </div>
                <div className="flex items-center">
                  <span className="text-green-700 font-medium w-24">Email:</span>
                ...
                <div className="flex items-center">
                  <span className="text-green-700 font-medium w-24">Points:</span>
                  <span className="text-gray-800">{session.user.point_total} (Remain: {session.user.point_remain})</span>
                </div>
                <div className="flex items-center">
                  <span className="text-green-700 font-medium w-24">Address:</span>
                  <span className="text-gray-800">{session.user.address}</span>
                </div>
                <div className="flex items-center">
                  <span className="text-green-700 font-medium w-24">Admin:</span>
                  <span className="text-gray-800">{session.user.is_admin ? "Yes" : "No"}</span>
                </div>
              </div>
            </div>
        ...
        <section className="bg-white shadow-lg rounded-lg p-6 relative overflow-hidden">
          ...
          {subActivities.length === 0 ? (
            <div className="bg-blue-50 p-8 rounded-lg text-center">
              ...
              <p className="mt-4 text-blue-700 font-medium">Loading activities...</p>
            </div>
          ) : (
              ...
              {subActivities.map((item) => (
                ...
                >
                  <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-3 text-center">
                    <h3 className="font-medium mb-2 text-sm text-purple-800 group-hover:text-purple-900">{item.sub_activity_name}</h3>
                    ...
                    <p className="mt-2 text-xs text-purple-600">ID: {item.sub_activity_id}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <footer className="text-center text-gray-500 text-sm py-4">
          <p>© {new Date().getFullYear()} QR Check-in System. All rights reserved.</p>
        </footer>
      </div>
    </div>
  );
